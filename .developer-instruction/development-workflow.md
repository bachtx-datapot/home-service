...
Bạn có thể thiết lập môi trường phát triển theo 2 cách: sử dụng **Dev Containers** (khuyên dùng) hoặc cài đặt thủ công.

### Cách 1: Sử dụng Dev Containers (Khuyên dùng)
Dự án hỗ trợ Dev Container cho 3 ngữ cảnh khác nhau. Yêu cầu máy đã cài Docker Desktop và VS Code extension "Dev Containers".

1.  **Root Dev Container** (Full-stack):
    *   Mở folder gốc của dự án trong VS Code.
    *   Nhấn `F1`, chọn **Dev Containers: Reopen in Container**.
    *   Môi trường này có sẵn Python 3.10, Node.js 24, Git để làm việc với cả backend và frontend.

2.  **Backend Dev Container** (Backend-focused):
    *   Mở folder `backend/` trong VS Code (File -> Open Folder...).
    *   Nhấn `F1`, chọn **Dev Containers: Reopen in Container**.
    *   Môi trường này tự động chạy Postgres database đi kèm và cài đặt sẵn `uv` để quản lý dependencies.

3.  **Frontend Dev Container** (Frontend-focused):
    *   Mở folder `frontend/` trong VS Code (File -> Open Folder...).
    *   Nhấn `F1`, chọn **Dev Containers: Reopen in Container**.
    *   Môi trường này có sẵn Node.js 24 và các công cụ linting.

### Cách 2: Setup Thủ công với Docker Compose
